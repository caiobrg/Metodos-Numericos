...
public:
  ...
  void configurarTerminal() {
    ...
    TInf = capturarInput(TInf, "Temperatua padrão do escoamento [K]");
...
public:
  ...
  void build_mesh() {
    ...
    // Constróid a malha com as temperaturas inicializadas em 0 K
    size_t Id = 0;
    // Varredura em y
    for (double pos_y = 0; pos_y < Ny; pos_y++) {
      ...
      for (double pos_x = 0; pos_x < Nx; pos_x++) {
        ...
        meshnodes.push_back(
            Node(Id, x, y)); // Adiciona o nó ao verotr de coordenada
        Id++;
      }
    }
  }
  ...
  // comentários mais completos se referir ao metodo south()
  ...
  // Mesh.west() retorna todos os ids dosnós da fronteira Oeste da malha, para
  // comentários mais completos se referir ao metodo south()
  std::vector<int> west() {
    int Ny = config.get_Ny();
    int Nx = config.get_Nx();
...
private:
  ...
  public:
    ...
    void dirichlet(const std::vector<double> &Tb_vector) {
      ...
      // Verifica se o vetore tem o mesmo tamanho do número de IDs alvo, evita
    ...
    // Sobrecaarga 3: Robin com valor único uniforme uniforme
...
public:
  ...
  SolverResult eliminacao_gaussiana() {
    ...
    for (int k = 0; k < totalSize - 1; k++) {
      ...
      // Inicia as váriaveis com o valor sem pivoteamento
      int max_index = k;
